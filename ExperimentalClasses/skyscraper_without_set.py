...
This is the Skyscraper class which does not use a set during the constraint propagation process.
...
class Skyscraper(Puzzle):
            ...
            if k != j:
              if value_to_remove in self.grid[r][k]:
                ...
                if len(self.grid[r][k]) == 1:
                  if not self.propagate_from_single_value(queue, r, k):
                    return False
            ...
            if k != j:
              if value_to_remove in self.grid[k][c]:
                ...
                if len(self.grid[k][c]) == 1:
                  if not self.propagate_from_single_value(queue, k, c):
                    return False
  ...
  def propagate_from_single_value(self, main_queue, row, col):
